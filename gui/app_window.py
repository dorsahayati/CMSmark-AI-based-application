...
class Worker(QtCore.QObject):
    ...
    def __init__(self, input_files, threshold, model_group, needs_normalization): # Add new argument
        super().__init__()
    ...
    def run(self):
        try:
            ...
            for i, csv_path in enumerate(self.input_files):
                ...
                cmsmark.perform_inference(csv_path, self.threshold, self.model_group, self.needs_normalization)

                # Determine the results path to send back to the UI
                input_file = Path(csv_path)
                final_results_path = str(input_file.parent / f"{input_file.stem}_results")
...
class AppWindow(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.model_group_label = QtWidgets.QLabel("Features Group:")
        self.groupCombo = QtWidgets.QComboBox()
        self.groupCombo.addItems(["Excellent", "Good", "Excellent-Good"])
        ...
        params_layout.addWidget(self.groupCombo, 1, 1)
    ...
    def run_inference(self):
        ...
        model_group = self.groupCombo.currentText()
        ...
        self.worker = Worker(self.input_files, threshold, model_group, normalize_data)
